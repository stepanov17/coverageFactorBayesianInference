...
public class CalcP_N {
    ...
    private double f(double x, double z, double gamma) {
        ...
        double tmp = (gamma * z + x) * (gamma * z + x) / (n - 1.) + 1.;
        return Math.pow(tmp, -0.5 * n) * Math.exp(-Math.pow(Math.abs(z / lambda), alpha));
    }
    ...
    private double getP(double gamma, double K, double fact) {
        ...
        double R = K * Math.sqrt((n - 1.) / (n - 3.) + gamma * gamma);
        double dx = 0.01 * R;
